...
global function lambert {
	parameter r1, r2, tof, mu, retrograde.
    ...
    // Flip direction is transfer angle is larger than 180 degrees XOR retrograde is true
    // WIP: Just gonna have to do both
    ...
    local x0 is initialGuess(lambda, t).
    local f is householdersMethod@:bind(lambda, t).
    local x is iterativeRootFinder(x0, f, 1e-5, 15).
...
local function householdersMethod {
    parameter lambda, t, x.

    local a is 1 / (1 - x ^ 2).
    local y is sqrt(1 - lambda ^ 2 * (1 - x ^ 2)).
    local tau is timeOfFlight(lambda, a, x).
...
local function timeOfFlight {
    ...
    if a > 0 {
        ...
        // A hyperbolic trajectory is extremely unlikely to be the lowest possible dV transfer but we calculate it anyway just to be thorough.
        set a to -a.
